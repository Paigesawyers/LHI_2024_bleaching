...
model_all_coral_cover <-
  glmmTMB(data = df_all_coral_cover,
          cbind(Coral, 
                Other) ~
            Month * Site +
            (1 | Site_transect),
...
# Save model output
write_rds(model_all_coral_cover,
...
model_live_coral_cover <-
  glmmTMB(data = df_live_coral_cover,
          cbind(Live_coral, 
                Other) ~
            Month * Site +
            (1 | Site_transect),
...
# Save model output
write_rds(model_live_coral_cover,
...
for (selected in c(genera_to_analyse, 
                   "other_genera")) {
  df_genera_count %>% 
  ...
  df_health_all_count %>% 
    ...
    glmmTMB(data = .,
            cbind(chosen, 
              ...
              (1 | Site_transect),
            family = "betabinomial") %>% 
    write_rds(paste0("./Models/model_health_all_betabinom_",
                     selected,
                     ".rds"))
}

## 5. Health status (within genera)) ----
# For each genus:
      ...
      df_selected %>% 
        ...
        glmmTMB(data = .,
                cbind(chosen, 
                  ...
                  (1 | Site_transect),
                family = "betabinomial") %>% 
        write_rds(paste0("./Models/model_health_specific_betabinom_",
                         genus,
                         "_",
                         selected,
                         ".rds"))
